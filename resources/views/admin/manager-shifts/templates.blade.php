                    ...
                    @if($preview)
                        ...
                        <div class="border rounded">
                            @foreach($preview['rows'] as $row)
                                <div class="preview-row {{ $row['has_conflict']?'is-conflict':'' }}"><strong>{{ \Carbon\Carbon::parse($row['date'])->format('D, M j') }}</strong><span>{{ $row['start_time'] }}–{{ $row['end_time'] }}</span><span>Role #{{ $row['item']->role_id }} · {{ $row['item']->employee_id ? 'Employee #'.$row['item']->employee_id : 'Open shift' }}</span><span class="preview-state">{{ $row['has_conflict']?'Conflict':'Ready' }}</span></div>
                            @endforeach
                        </div>
